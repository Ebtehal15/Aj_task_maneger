const STRINGS = {
  en: {
    appTitle: 'Task Manager',
    loginTitle: 'Sign In',
    username: 'Username',
    password: 'Password',
    loginButton: 'Login',
    logout: 'Logout',
    adminDashboard: 'Admin Dashboard',
    userTasks: 'My Tasks',
    myTasks: 'My Tasks',
    createTask: 'Create Task',
    taskTitle: 'Task Title',
    taskDescription: 'Description',
    description: 'Description',
    deadline: 'Estimated Completion Date',
    completedAt: 'Completion Date',
    createdAt: 'Creation Date',
    givenDate: 'Start Date',
    location: 'Location',
    assignedTo: 'Assign to',
    status: 'Status',
    status_pending: 'Pending',
    status_in_progress: 'In Progress',
    status_done: 'Completed',
    totalTasks: 'Total Tasks',
    completedTasks: 'Completed Tasks',
    pendingTasks: 'Pending Tasks',
    inProgressTasks: 'In Progress Tasks',
    urgentTasks: 'Urgent Tasks',
    mostAssignedUser: 'Most Assigned User',
    uploadFiles: 'Upload files',
    save: 'Save',
    addUser: 'Add User',
    addMunicipality: 'Add Municipality',
    municipalityAddedSuccessfully: 'Municipality added successfully',
    errorAddingMunicipality: 'Error adding municipality',
    addRegion: 'Add Region',
    regionAddedSuccessfully: 'Region added successfully',
    errorAddingRegion: 'Error adding region',
    addCity: 'Add City',
    cityAddedSuccessfully: 'City added successfully',
    errorAddingCity: 'Error adding city',
    translate: 'Translate',
    translateNoText: 'No text to translate.',
    translateFailed: 'Translation failed.',
    translateError: 'An error occurred while translating.',
    translateUnchanged:
      'The text did not change. The service may be blocked, or the text may already match the target language. Check the server console for [translate] messages.',
    optional: 'Optional',
    cancel: 'Cancel',
    pleaseFillRequiredFields: 'Please fill all required fields',
    userAddedSuccessfully: 'User added successfully',
    errorAddingUser: 'Error adding user',
    delete: 'Delete',
    deleteUser: 'Delete User',
    confirmDeleteUser: 'Are you sure you want to delete user',
    errorDeletingUser: 'Error deleting user',
    filterByUser: 'Filter by user',
    filterByStatus: 'Filter by status',
    filterByCity: 'Filter by city',
    filterByDate: 'Filter by date',
    filterByGivenDate: 'Given Date',
    filterByCompletedDate: 'Completion Date',
    filterByDepartment: 'Department',
    filterByArchive: 'Archive',
    filterByMunicipality: 'Filter by municipality',
    filterByRegion: 'Filter by region',
    selectFilterType: 'Select filter type',
    filters: 'Filters',
    personnel: 'Personnel',
    municipality: 'Municipality',
    region: 'Region',
    city: 'City',
    date: 'Date',
    resetFilters: 'Reset Filters',
    exportExcel: 'Export to Excel',
    noTasks: 'No tasks found',
    noFiles: 'No files uploaded',
    languageEnglish: 'English',
    languageArabic: 'Arabic',
    languageTurkish: 'Turkish',
    invalidCredentials: 'Invalid username or password',
    back: 'Back',
    edit: 'Edit',
    createdBy: 'Created by',
    adminLabel: 'Admin',
    view: 'View',
    team: 'Team',
    responsible1: 'Responsible 1',
    responsible2: 'Responsible 2',
    responsible3: 'Responsible 3',
    filesHelpText: 'Files can be uploaded by both admin and staff when creating/updating tasks.',
    recentUpdates: 'Recent Updates',
    noUpdates: 'No updates yet.',
    statusLabel: 'Status',
    usersPageTitle: 'Users',
    addUser: 'Add User',
    searchUsersPlaceholder: 'Search users...',
    existingUsers: 'Existing Users',
    role: 'Role',
    role_user: 'User',
    role_super_admin: 'Super Admin',
    role_admin: 'Admin',
    role_system_admin: 'System Administrator',
    role_system_admin_hint: 'This role is assigned only in code/database, not from the UI.',
    systemAdminsPageTitle: 'System Admins',
    auditLogPageTitle: 'Audit log',
    auditLogFilterAction: 'Action contains',
    auditLogFilterUsername: 'User',
    auditLogDateFrom: 'From date',
    auditLogDateTo: 'To date',
    auditLogApplyFilters: 'Apply',
    auditLogNoEntries: 'No audit entries yet.',
    auditLogColTime: 'Time',
    auditLogColUser: 'User',
    auditLogColPerformedBy: 'Performed by',
    auditLogColAction: 'Action',
    auditLogColEntity: 'Entity',
    auditLogColEntityId: 'ID',
    auditLogColDetails: 'Details',
    auditLogColIp: 'IP',
    auditLogColUserAgent: 'User agent',
    auditLogTotalRows: 'Total',
    auditLogPrev: 'Previous',
    auditLogNext: 'Next',
    auditLogDelete: 'Delete',
    auditLogDeleteConfirm: 'Delete this audit log entry?',
    auditLogDeleteModalTitle: 'Remove audit log entry',
    auditLogDeleteModalHint: 'This action cannot be undone.',
    noUsers: 'No users found',
    notificationsTitle: 'Notifications',
    noNotifications: 'No new notifications.',
    viewTask: 'View Task',
    email: 'Email',
    emailHelpText: 'Email address for notifications (optional)',
    editTask: 'Edit Task',
    notePlaceholder: 'Work done / description (optional)',
    makeReport: 'Make a report',
    notification_task_assigned: 'New task assigned by {by}: {title}',
    notification_task_updated: 'Staff ({by}) updated task "{title}" - Status: {status} - Note: {note} - Attachment: {fileCount} file(s) uploaded',
    notification_task_updated_no_note: 'Staff ({by}) updated task "{title}" - Status: {status} - Attachment: {fileCount} file(s) uploaded',
    notification_task_updated_no_file: 'Staff ({by}) updated task "{title}" - Status: {status} - Note: {note}',
    notification_task_updated_simple: 'Staff ({by}) updated task "{title}" - Status: {status}',
    status_completed: 'Completed',
    status_in_progress_text: 'In Progress',
    status_pending_text: 'Pending',
    unread: 'Unread',
    read: 'Read',
    home: 'Home',
    dashboard: 'Dashboard',
    menu: 'Menu',
    tasks: 'Tasks',
    reports: 'Reports',
    userReports: 'User Reports',
    selectUserForReport: 'Select a user to view their tasks',
    selectUsers: 'Select users',
    showSelectedUsers: 'Show selected',
    resultsCount: 'Results',
    tarih: 'Date',
    konuSorumlusu: 'Subject Responsible',
    sorumlu: 'Responsible',
    sorumlu2: 'Responsible 2',
    sorumlu3: 'Responsible 3',
    bolge: 'Region',
    il: 'City',
    belediye: 'Municipality',
    departman: 'Department',
    arsiv: 'Archive',
    arsivVar: 'Yes',
    arsivYok: 'No',
    verilenIsTarihi: 'Given Job Date',
    tahminiIsBitistarihi: 'Estimated Job End Date',
    statusOnemli: 'Important',
    statusDevamEdiyor: 'In Progress',
    statusBitti: 'Completed',
    departmanSaha: 'Field',
    departmanBelediye: 'Municipality',
    departmanHukuk: 'Legal',
    departmanLisans: 'Licensing',
    departmanGenel: 'General',
    departmanInsanKaynaklari: 'Human Resources',
    isUrgent: 'Is Urgent?',
    urgent: 'Urgent',
    notUrgent: 'Not Urgent',
    shareFile: 'Share File',
    shareViaEmail: 'Share via Email',
    shareViaWhatsApp: 'Share via WhatsApp',
    shareViaLink: 'Copy Link',
    linkCopied: 'Link copied to clipboard!',
    shareFileTitle: 'Share Excel File',
    fileDownloaded: 'File downloaded',
    shareDownloadedFile: 'Share downloaded file',
    taskDetailsTitle: 'Task Details',
    basicInfo: 'General Info',
    responsibleInfo: 'Responsibles',
    dateInfo: 'Dates',
    attachments: 'Attachments',
    taskDistribution: 'Task Distribution',
    charts: 'Charts',
    overdueCharts: 'Charts for Overdue Tasks',
    statusDistributionPie: 'Status Distribution (Pie)',
    departmentDistributionPie: 'Department Distribution (Pie)',
    overdueByUserChart: 'Overdue Tasks by User',
    overdueBySubjectOwnerChart: 'Overdue Tasks by Subject Owner',
    urgentOverdueByUserChart: 'Urgent & Overdue Tasks (by User)',
    overdueByTopicDaysChart: 'Overdue Days by Topic Titles',
    subjectOwnerLabel: 'Subject owner',
    daysOverdueSuffix: 'days overdue',
    noOverdueTopicFound: 'No overdue topic titles found.',
    overdueListSubviewTasks: 'Overdue tasks',
    overdueListSubviewDueSoon: 'Due soon (3 days)',
    overdueListSubviewReasons: 'Delay reasons',
    daysUntilDeadlineSuffix: 'days left',
    noDueSoonTasksFound: 'No open tasks due in the next 3 days.',
    deadlineDateLabel: 'Due date',
    overdueListNoteLabel: 'Delay reason',
    overdueListNoteAdd: 'Add note',
    overdueListNoteEdit: 'Edit',
    overdueListNoteCancel: 'Cancel',
    overdueListNoteBy: 'Saved by',
    overdueListNoteShow: 'View note',
    overdueListNoteHide: 'Hide note',
    overdueListNotePlaceholder: 'Write your note…',
    overdueListNoteSaved: 'Saved.',
    overdueListNoteSaveError: 'Could not save. Try again.',
    untitledTask: 'Untitled task',
    noData: 'No data',
    other: 'Other',
    loading: 'Loading...',
    cardView: 'Card View',
    tableView: 'Table View',
    evaluation: 'Evaluation',
    completionRate: 'Completion Rate',
    performanceScore: 'Performance score',
    overdueTasks: 'Overdue Tasks',
    yoneticiKontrol: 'Manager Tracking',
    yoneticiKontrolEvet: 'Yes',
    yoneticiKontrolHayir: 'No'
  },
  tr: {
    appTitle: 'Görev Yönetimi',
    loginTitle: 'Giriş Yap',
    username: 'Kullanıcı Adı',
    password: 'Şifre',
    loginButton: 'Giriş',
    logout: 'Çıkış',
    adminDashboard: 'Yönetici Paneli',
    tasks: 'Görevler',
    userTasks: 'Görevlerim',
    myTasks: 'Görevlerim',
    createTask: 'Görev Oluştur',
    taskTitle: 'Görev Başlığı',
    taskSubject: 'İş Konusu',
    taskDescription: 'Açıklama',
    description: 'Açıklama',
    deadline: 'Tahmini İş Bitiş Tarihi',
    completedAt: 'Bitiş Tarihi',
    createdAt: 'Oluşturma Tarihi',
    givenDate: 'İş Başlama Tarihi',
    location: 'Lokasyon',
    assignedTo: 'Atanan',
    status: 'Durum',
    status_pending: 'Beklemede',
    status_in_progress: 'Devam Ediyor',
    status_done: 'Tamamlandı',
    totalTasks: 'Toplam Görev',
    completedTasks: 'Tamamlanan Görevler',
    pendingTasks: 'Beklemede Olan Görevler',
    inProgressTasks: 'Devam Eden Görevler',
    urgentTasks: 'Acil Görevler',
    mostAssignedUser: 'En Çok Görev Alan Personel',
    uploadFiles: 'Dosya Yükle',
    save: 'Kaydet',
    cancel: 'İptal',
    filterByUser: 'Kullanıcıya göre filtrele',
    filterByStatus: 'Duruma göre filtrele',
    filterByCity: 'İle göre filtrele',
    filterByDate: 'Tarihe göre filtrele',
    filterByGivenDate: 'İş Verilme Tarihi',
    filterByCompletedDate: 'İş Bitiş Tarihi',
    filterByDepartment: 'Departman',
    filterByArchive: 'Arşiv',
    filterByMunicipality: 'Belediyeye göre filtrele',
    filterByRegion: 'Bölgeye göre filtrele',
    selectFilterType: 'Filtre tipi seçin',
    filters: 'Filtreler',
    personnel: 'Personel',
    municipality: 'Belediye',
    region: 'Bölge',
    city: 'İl',
    date: 'Tarih',
    resetFilters: 'Filtreleri Sıfırla',
    exportExcel: 'Excel\'e Aktar',
    resultsCount: 'Sonuç sayısı',
    noTasks: 'Görev bulunamadı',
    noFiles: 'Henüz dosya yok',
    languageEnglish: 'İngilizce',
    languageArabic: 'Arapça',
    languageTurkish: 'Türkçe',
    invalidCredentials: 'Geçersiz kullanıcı adı veya şifre',
    back: 'Geri',
    edit: 'Düzenle',
    createdBy: 'Oluşturan',
    adminLabel: 'Yönetici',
    team: 'Ekip',
    responsible1: 'Sorumlu 1',
    responsible2: 'Sorumlu 2',
    responsible3: 'Sorumlu 3',
    view: 'Görüntüle',
    filesHelpText: 'Dosyalar görev oluştururken veya güncellerken hem yönetici hem de personel tarafından yüklenebilir.',
    recentUpdates: 'Son Güncellemeler',
    noUpdates: 'Henüz güncelleme yok.',
    statusLabel: 'Durum',
    usersPageTitle: 'Kullanıcılar',
    addUser: 'Kullanıcı Ekle',
    searchUsersPlaceholder: 'Kullanıcı ara...',
    addMunicipality: 'Belediye Ekle',
    municipalityAddedSuccessfully: 'Belediye başarıyla eklendi',
    errorAddingMunicipality: 'Belediye eklenirken hata oluştu',
    addRegion: 'Bölge Ekle',
    regionAddedSuccessfully: 'Bölge başarıyla eklendi',
    errorAddingRegion: 'Bölge eklenirken hata oluştu',
    addCity: 'İl Ekle',
    cityAddedSuccessfully: 'İl başarıyla eklendi',
    errorAddingCity: 'İl eklenirken hata oluştu',
    translate: 'Çevir',
    translateNoText: 'Çevrilecek metin yok.',
    translateFailed: 'Çeviri başarısız oldu.',
    translateError: 'Çeviri sırasında hata oluştu.',
    translateUnchanged:
      'Metin değişmedi. Çeviri servisi engellenmiş olabilir veya metin zaten hedef dilde olabilir. Sunucu konsolunda [translate] kayıtlarına bakın.',
    existingUsers: 'Mevcut Kullanıcılar',
    role: 'Rol',
    role_user: 'Kullanıcı',
    role_super_admin: 'Süper Yönetici',
    role_admin: 'Yönetici',
    role_system_admin: 'Sistem Yöneticisi',
    role_system_admin_hint: 'Bu rol yalnızca kod/veritabanı tarafından atanır, arayüzden atanamaz.',
    systemAdminsPageTitle: 'Sistem Yöneticileri',
    auditLogPageTitle: 'Denetim günlüğü',
    auditLogFilterAction: 'İşlem (içerir)',
    auditLogFilterUsername: 'Kullanıcı',
    auditLogDateFrom: 'Başlangıç tarihi',
    auditLogDateTo: 'Bitiş tarihi',
    auditLogApplyFilters: 'Uygula',
    auditLogNoEntries: 'Henüz denetim kaydı yok.',
    auditLogColTime: 'Zaman',
    auditLogColUser: 'Kullanıcı',
    auditLogColPerformedBy: 'Kimlik',
    auditLogColAction: 'İşlem',
    auditLogColEntity: 'Varlık',
    auditLogColEntityId: 'Kimlik',
    auditLogColDetails: 'Ayrıntı',
    auditLogColIp: 'IP',
    auditLogColUserAgent: 'Tarayıcı',
    auditLogTotalRows: 'Toplam',
    auditLogPrev: 'Önceki',
    auditLogNext: 'Sonraki',
    auditLogDelete: 'Sil',
    auditLogDeleteConfirm: 'Bu denetim kaydını silmek istediğinize emin misiniz?',
    auditLogDeleteModalTitle: 'Denetim kaydını sil',
    auditLogDeleteModalHint: 'Bu işlem geri alınamaz.',
    noUsers: 'Kullanıcı bulunamadı',
    notificationsTitle: 'Bildirimler',
    noNotifications: 'Yeni bildirim yok.',
    viewTask: 'Görevi Görüntüle',
    email: 'E-posta',
    emailHelpText: 'Bildirimler için e-posta adresi (isteğe bağlı)',
    editTask: 'Görevi Düzenle',
    notePlaceholder: 'Yapılan iş / açıklama (isteğe bağlı)',
    makeReport: 'Rapor oluştur',
    notification_task_assigned: '{by} tarafından yeni görev atandı: {title}',
    notification_task_updated: 'Personel ({by}) "{title}" görevini güncelledi - Durum: {status} - Not: {note} - Ek: {fileCount} dosya yüklendi',
    notification_task_updated_no_note: 'Personel ({by}) "{title}" görevini güncelledi - Durum: {status} - Ek: {fileCount} dosya yüklendi',
    notification_task_updated_no_file: 'Personel ({by}) "{title}" görevini güncelledi - Durum: {status} - Not: {note}',
    notification_task_updated_simple: 'Personel ({by}) "{title}" görevini güncelledi - Durum: {status}',
    status_completed: 'Tamamlandı',
    status_in_progress_text: 'Devam Ediyor',
    status_pending_text: 'Beklemede',
    unread: 'Okunmamış',
    read: 'Okundu',
    home: 'Ana Sayfa',
    dashboard: 'Kontrol Paneli',
    menu: 'Menü',
    reports: 'Raporlar',
    userReports: 'Kullanıcı Raporları',
    selectUserForReport: 'Görevlerini görmek için bir kullanıcı seçin',
    selectUsers: 'Kullanıcı seç',
    showSelectedUsers: 'Seçilenleri göster',
    tarih: 'Tarih',
    konuSorumlusu: 'Konu Sorumlusu',
    sorumlu: 'Sorumlu',
    sorumlu2: 'Sorumlu 2',
    sorumlu3: 'Sorumlu 3',
    bolge: 'Bölge',
    il: 'İl',
    belediye: 'Belediye',
    departman: 'Departman',
    arsiv: 'Arşiv',
    arsivVar: 'Var',
    arsivYok: 'Yok',
    verilenIsTarihi: 'Verilen İş Tarihi',
    tahminiIsBitistarihi: 'Tahmini İş Bitiş Tarihi',
    statusOnemli: 'Önemli',
    statusDevamEdiyor: 'Devam Ediyor',
    statusBitti: 'Bitti',
    departmanSaha: 'Saha',
    departmanBelediye: 'Belediye',
    departmanHukuk: 'Hukuk',
    departmanLisans: 'Lisans',
    departmanGenel: 'Genel',
    departmanInsanKaynaklari: 'İnsan Kaynakları',
    isUrgent: 'Acil mi?',
    urgent: 'Acil',
    notUrgent: 'Acil Değil',
    shareFile: 'Dosyayı Paylaş',
    shareViaEmail: 'E-posta ile Paylaş',
    shareViaWhatsApp: 'WhatsApp ile Paylaş',
    shareViaLink: 'Linki Kopyala',
    linkCopied: 'Link panoya kopyalandı!',
    shareFileTitle: 'Excel Dosyasını Paylaş',
    fileDownloaded: 'Dosya indirildi',
    shareDownloadedFile: 'İndirilen dosyayı paylaş',
    taskDetailsTitle: 'Görev Detayları',
    basicInfo: 'Genel Bilgiler',
    responsibleInfo: 'Sorumlular',
    dateInfo: 'Tarihler',
    attachments: 'Ekler',
    taskDistribution: 'Görev Dağılımı',
    charts: 'Grafikler',
    overdueCharts: 'Geciken Görevler için Grafikler',
    statusDistributionPie: 'Duruma göre dağılım',
    departmentDistributionPie: 'Departmana göre dağılım',
    overdueByUserChart: 'Kişiye göre geciken görevler',
    overdueBySubjectOwnerChart: 'Konu sorumlusuna göre geciken görevler',
    urgentOverdueByUserChart: 'Acil & geciken görevler (kişiye göre)',
    overdueByTopicDaysChart: 'Konu başlıklarına göre gecikme günü',
    subjectOwnerLabel: 'Konu sorumlusu',
    daysOverdueSuffix: 'gün gecikme',
    noOverdueTopicFound: 'Geciken konu başlığı bulunamadı.',
    overdueListSubviewTasks: 'Geciken görevler',
    overdueListSubviewDueSoon: 'Yaklaşan deadline',
    overdueListSubviewReasons: 'Gecikme sebepleri',
    daysUntilDeadlineSuffix: 'gün kaldı',
    noDueSoonTasksFound: 'Önümüzdeki 3 gün içinde bitecek açık görev yok.',
    deadlineDateLabel: 'Bitiş tarihi',
    overdueListNoteLabel: 'Gecikme sebebi',
    overdueListNoteAdd: 'Not ekle',
    overdueListNoteEdit: 'Düzenle',
    overdueListNoteCancel: 'İptal',
    overdueListNoteBy: 'Kaydeden',
    overdueListNoteShow: 'Notu gör',
    overdueListNoteHide: 'Notu gizle',
    overdueListNotePlaceholder: 'Notunuzu yazın…',
    overdueListNoteSaved: 'Kaydedildi.',
    overdueListNoteSaveError: 'Kaydedilemedi. Tekrar deneyin.',
    untitledTask: 'Başlıksız görev',
    noData: 'Veri yok',
    other: 'Diğer',
    loading: 'Yükleniyor...',
    cardView: 'Kart Görünümü',
    tableView: 'Tablo Görünümü',
    evaluation: 'Değerlendirme',
    completionRate: 'Tamamlama oranı',
    performanceScore: 'Performans skoru',
    overdueTasks: 'Geciken Görevler',
    yoneticiKontrol: 'Yönetici Takip',
    yoneticiKontrolEvet: 'Evet',
    yoneticiKontrolHayir: 'Hayır'
  },
  ar: {
    appTitle: 'نظام إدارة المهام',
    loginTitle: 'تسجيل الدخول',
    username: 'اسم المستخدم',
    password: 'كلمة المرور',
    loginButton: 'دخول',
    logout: 'تسجيل الخروج',
    adminDashboard: 'لوحة التحكم (المدير)',
    userTasks: 'مهامي',
    myTasks: 'مهامي',
    createTask: 'إنشاء مهمة',
    taskTitle: 'عنوان المهمة',
    taskSubject: 'موضوع العمل',
    taskDescription: 'وصف المهمة',
    description: 'الوصف',
    deadline: 'تاريخ انتهاء العمل المتوقع',
    completedAt: 'تاريخ الانتهاء',
    createdAt: 'تاريخ الإنشاء',
    givenDate: 'تاريخ بدء العمل',
    location: 'الموقع',
    assignedTo: 'تعيين إلى',
    status: 'الحالة',
    status_pending: 'قيد الانتظار',
    status_in_progress: 'قيد التنفيذ',
    status_done: 'مكتملة',
    totalTasks: 'إجمالي المهام',
    uploadFiles: 'رفع الملفات',
    save: 'حفظ',
    cancel: 'إلغاء',
    filterByUser: 'تصفية حسب الموظف',
    filterByStatus: 'تصفية حسب الحالة',
    filterByCity: 'تصفية حسب المحافظة',
    filterByDate: 'تصفية حسب التاريخ',
    filterByGivenDate: 'تاريخ الإعطاء',
    filterByCompletedDate: 'تاريخ انتهاء العمل',
    filterByDepartment: 'القسم',
    filterByArchive: 'الأرشيف',
    filterByMunicipality: 'تصفية حسب البلدية',
    filterByRegion: 'تصفية حسب المنطقة',
    selectFilterType: 'اختر نوع التصفية',
    filters: 'التصفية',
    personnel: 'الموظف',
    municipality: 'البلدية',
    region: 'المنطقة',
    city: 'المحافظة',
    date: 'التاريخ',
    resetFilters: 'إعادة تعيين المرشحات',
    exportExcel: 'تصدير إلى Excel',
    noTasks: 'لا توجد مهام',
    noFiles: 'لا توجد ملفات',
    languageEnglish: 'الإنجليزية',
    languageArabic: 'العربية',
    languageTurkish: 'التركية',
    invalidCredentials: 'اسم المستخدم أو كلمة المرور غير صحيحة',
    back: 'رجوع',
    edit: 'تعديل',
    createdBy: 'أنشئت بواسطة',
    adminLabel: 'المدير',
    team: 'الفريق',
    responsible1: 'المسؤول 1',
    responsible2: 'المسؤول 2',
    responsible3: 'المسؤول 3',
    view: 'عرض',
    filesHelpText: 'يمكن للمدير والموظفين رفع الملفات عند إنشاء أو تحديث المهمة.',
    recentUpdates: 'آخر التحديثات',
    noUpdates: 'لا توجد تحديثات مسجلة بعد.',
    statusLabel: 'الحالة',
    usersPageTitle: 'المستخدمون',
    addUser: 'إضافة مستخدم',
    searchUsersPlaceholder: 'ابحث عن مستخدم...',
    addMunicipality: 'إضافة بلدية',
    municipalityAddedSuccessfully: 'تمت إضافة البلدية بنجاح',
    errorAddingMunicipality: 'حدث خطأ أثناء إضافة البلدية',
    addRegion: 'إضافة منطقة',
    regionAddedSuccessfully: 'تمت إضافة المنطقة بنجاح',
    errorAddingRegion: 'حدث خطأ أثناء إضافة المنطقة',
    addCity: 'إضافة محافظة',
    cityAddedSuccessfully: 'تمت إضافة المحافظة بنجاح',
    errorAddingCity: 'حدث خطأ أثناء إضافة المحافظة',
    translate: 'ترجمة',
    translateNoText: 'لا يوجد نص للترجمة.',
    translateFailed: 'فشلت الترجمة.',
    translateError: 'حدث خطأ أثناء الترجمة.',
    translateUnchanged:
      'لم يتغير النص. قد تكون خدمة الترجمة محظورة أو النص بالفعل باللغة المستهدفة. راجع سجل الخادم لرسائل [translate].',
    existingUsers: 'المستخدمون الحاليون',
    role: 'الدور',
    role_user: 'موظف',
    role_super_admin: 'مدير رئيسي',
    role_admin: 'مدير',
    role_system_admin: 'مدير النظام',
    role_system_admin_hint: 'هذه الصلاحية تُعيّن من الكود/قاعدة البيانات فقط، وليس من الواجهة.',
    systemAdminsPageTitle: 'مدراء النظام',
    auditLogPageTitle: 'سجل التدقيق',
    auditLogFilterAction: 'الإجراء (يحتوي)',
    auditLogFilterUsername: 'المستخدم',
    auditLogDateFrom: 'من تاريخ',
    auditLogDateTo: 'إلى تاريخ',
    auditLogApplyFilters: 'تطبيق',
    auditLogNoEntries: 'لا توجد سجلات تدقيق بعد.',
    auditLogColTime: 'الوقت',
    auditLogColUser: 'المستخدم',
    auditLogColPerformedBy: 'من نفّذ',
    auditLogColAction: 'الإجراء',
    auditLogColEntity: 'الكيان',
    auditLogColEntityId: 'المعرّف',
    auditLogColDetails: 'التفاصيل',
    auditLogColIp: 'عنوان IP',
    auditLogColUserAgent: 'وكيل المستخدم',
    auditLogTotalRows: 'الإجمالي',
    auditLogPrev: 'السابق',
    auditLogNext: 'التالي',
    auditLogDelete: 'حذف',
    auditLogDeleteConfirm: 'هل تريد حذف سجل التدقيق هذا؟',
    auditLogDeleteModalTitle: 'حذف سجل التدقيق',
    auditLogDeleteModalHint: 'لا يمكن التراجع عن هذا الإجراء.',
    noUsers: 'لا يوجد مستخدمون',
    notificationsTitle: 'الإشعارات',
    noNotifications: 'لا توجد إشعارات جديدة.',
    viewTask: 'عرض المهمة',
    email: 'البريد الإلكتروني',
    emailHelpText: 'عنوان البريد الإلكتروني للإشعارات (اختياري)',
    editTask: 'تعديل المهمة',
    notePlaceholder: 'الوصف / ما تم إنجازه (اختياري)',
    makeReport: 'إنشاء تقرير',
    notification_task_assigned: 'تم تعيين مهمة جديدة بواسطة {by}: {title}',
    notification_task_updated: 'قام الموظف ({by}) بتحديث المهمة \"{title}\" - الحالة: {status} - الملاحظة: {note} - المرفق: تم رفع {fileCount} ملف',
    notification_task_updated_no_note: 'قام الموظف ({by}) بتحديث المهمة \"{title}\" - الحالة: {status} - المرفق: تم رفع {fileCount} ملف',
    notification_task_updated_no_file: 'قام الموظف ({by}) بتحديث المهمة \"{title}\" - الحالة: {status} - الملاحظة: {note}',
    notification_task_updated_simple: 'قام الموظف ({by}) بتحديث المهمة \"{title}\" - الحالة: {status}',
    status_completed: 'مكتملة',
    status_in_progress_text: 'قيد التنفيذ',
    status_pending_text: 'قيد الانتظار',
    unread: 'غير مقروء',
    read: 'مقروء',
    home: 'الرئيسية',
    dashboard: 'لوحة التحكم',
    menu: 'القائمة',
    tasks: 'المهام',
    reports: 'التقارير',
    userReports: 'تقارير المستخدمين',
    selectUserForReport: 'اختر مستخدمًا لعرض مهامه',
    selectUsers: 'اختر المستخدمين',
    showSelectedUsers: 'عرض المحدد',
    resultsCount: 'عدد النتائج',
    totalTasks: 'إجمالي المهام',
    completedTasks: 'المهام المكتملة',
    pendingTasks: 'المهام المعلقة',
    inProgressTasks: 'المهام قيد التنفيذ',
    urgentTasks: 'المهام العاجلة',
    mostAssignedUser: 'أكثر موظف تم تعيين المهام له',
    tarih: 'التاريخ',
    konuSorumlusu: 'مسؤول الموضوع',
    sorumlu: 'المسؤول',
    sorumlu2: 'المسؤول 2',
    sorumlu3: 'المسؤول 3',
    bolge: 'المنطقة',
    il: 'المدينة',
    belediye: 'البلدية',
    departman: 'القسم',
    arsiv: 'الأرشيف',
    arsivVar: 'نعم',
    arsivYok: 'لا',
    verilenIsTarihi: 'تاريخ إعطاء العمل',
    tahminiIsBitistarihi: 'تاريخ انتهاء العمل المتوقع',
    statusOnemli: 'مهم',
    statusDevamEdiyor: 'قيد التنفيذ',
    statusBitti: 'انتهى',
    departmanSaha: 'الميدان',
    departmanBelediye: 'البلدية',
    departmanHukuk: 'القانون',
    departmanLisans: 'التراخيص',
    departmanGenel: 'عام',
    departmanInsanKaynaklari: 'الموارد البشرية',
    isUrgent: 'هل هو عاجل؟',
    urgent: 'عاجل',
    notUrgent: 'غير عاجل',
    shareFile: 'مشاركة الملف',
    shareViaEmail: 'مشاركة عبر البريد الإلكتروني',
    shareViaWhatsApp: 'مشاركة عبر واتساب',
    shareViaLink: 'نسخ الرابط',
    linkCopied: 'تم نسخ الرابط إلى الحافظة!',
    shareFileTitle: 'مشاركة ملف Excel',
    fileDownloaded: 'تم تنزيل الملف',
    shareDownloadedFile: 'مشاركة الملف الذي تم تنزيله',
    taskDetailsTitle: 'تفاصيل المهمة',
    basicInfo: 'معلومات عامة',
    responsibleInfo: 'المسؤولون',
    dateInfo: 'التواريخ',
    attachments: 'المرفقات',
    taskDistribution: 'توزيع المهام',
    charts: 'الرسوم البيانية',
    overdueCharts: 'رسوم المهام المتأخرة',
    statusDistributionPie: 'توزيع الحالات (دائري)',
    departmentDistributionPie: 'توزيع الأقسام (دائري)',
    overdueByUserChart: 'المهام المتأخرة حسب المستخدم',
    overdueBySubjectOwnerChart: 'المهام المتأخرة حسب مسؤول الموضوع',
    urgentOverdueByUserChart: 'المهام العاجلة والمتأخرة (حسب المستخدم)',
    overdueByTopicDaysChart: 'أيام التأخير حسب عناوين المواضيع',
    subjectOwnerLabel: 'مسؤول الموضوع',
    daysOverdueSuffix: 'يوم تأخير',
    noOverdueTopicFound: 'لا توجد عناوين مواضيع متأخرة.',
    overdueListSubviewTasks: 'المهام المتأخرة',
    overdueListSubviewDueSoon: 'المستحقة قريباً',
    overdueListSubviewReasons: 'أسباب التأخير',
    daysUntilDeadlineSuffix: 'يوم متبقي',
    noDueSoonTasksFound: 'لا توجد مهام مفتوحة مستحقة خلال 3 أيام.',
    deadlineDateLabel: 'تاريخ الاستحقاق',
    overdueListNoteLabel: 'سبب التأخير',
    overdueListNoteAdd: 'إضافة ملاحظة',
    overdueListNoteEdit: 'تعديل',
    overdueListNoteCancel: 'إلغاء',
    overdueListNoteBy: 'حفظ بواسطة',
    overdueListNoteShow: 'عرض الملاحظة',
    overdueListNoteHide: 'إخفاء الملاحظة',
    overdueListNotePlaceholder: 'اكتب ملاحظتك…',
    overdueListNoteSaved: 'تم الحفظ.',
    overdueListNoteSaveError: 'تعذر الحفظ. حاول مرة أخرى.',
    untitledTask: 'مهمة بدون عنوان',
    noData: 'لا توجد بيانات',
    other: 'أخرى',
    loading: 'جاري التحميل...',
    cardView: 'عرض البطاقات',
    tableView: 'عرض الجدول',
    evaluation: 'التقييم',
    completionRate: 'نسبة الإنجاز',
    performanceScore: 'نقاط الأداء',
    overdueTasks: 'المهام المتأخرة',
    yoneticiKontrol: 'متابعة إدارية',
    yoneticiKontrolEvet: 'نعم',
    yoneticiKontrolHayir: 'لا'
  }
};

function getI18nMiddleware() {
  return (req, res, next) => {
    try {
      const sessionLang = req.session?.lang;
      let lang = 'ar'; // Varsayılan dil Arapça
      if (sessionLang === 'en') {
        lang = 'en';
      } else if (sessionLang === 'tr') {
        lang = 'tr';
      } else if (sessionLang === 'ar') {
        lang = 'ar';
      }
      // Eğer session'da dil yoksa, varsayılan olarak Arapça ayarla
      if (!sessionLang && req.session) {
        req.session.lang = 'ar';
      }
      req.lang = lang;
      req.dir = lang === 'ar' ? 'rtl' : 'ltr';
      req.t = (key) => STRINGS[lang]?.[key] || key;
      res.locals.t = req.t;
      res.locals.lang = lang;
      res.locals.dir = req.dir;
      res.locals.currentUser = req.user;
      next();
    } catch (error) {
      console.error('⚠️  Error in i18n middleware (non-fatal):', error.message);
      // Fallback to default values
      req.lang = 'ar';
      req.dir = 'rtl';
      req.t = (key) => key;
      res.locals.t = req.t;
      res.locals.lang = 'ar';
      res.locals.dir = 'rtl';
      res.locals.currentUser = req.user;
      next();
    }
  };
}

module.exports = {
  getI18nMiddleware
};


